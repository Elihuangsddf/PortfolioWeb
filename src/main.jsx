...
const copy = {
  es: {
    ...
    documents: { syncing: 'Sincronizando con Google Drive…', live: 'Sincronizado con Google Drive', unavailable: 'Google Drive no disponible', automatic: 'Actualización automática al cargar', empty: 'Aún no hay archivos en esta carpeta.', open: 'Abrir archivo', folder: 'Abrir carpeta en Drive', updated: 'Actualizado', preview: 'Vista previa de' },
  ...
  en: {
    ...
    documents: { syncing: 'Syncing with Google Drive…', live: 'Synced with Google Drive', unavailable: 'Google Drive unavailable', automatic: 'Automatically updated on load', empty: 'There are no files in this folder yet.', open: 'Open file', folder: 'Open folder in Drive', updated: 'Updated', preview: 'Preview of' },
...
function DocumentSection({ id, content, files, status, folderUrl, labels, formatDate, tone }) {
  return <section id={id} className={`section documents-section documents-${tone}`}>
    <div className="container">
      ...
      <div className="documents-heading reveal"><div><h2>{content.title}<br /><em>{content.accent}</em></h2><p>{content.intro}</p></div><div className={`drive-sync ${status}`}><span className="sync-dot" />{status === 'syncing' ? labels.syncing : status === 'live' ? labels.live : labels.unavailable}<small>{labels.automatic}</small></div></div>
    ...
    <div ref={page} className="site-shell" onPointerMove={handlePointerMove}>
      ...
      <main>
        <section id="inicio" className="hero container"><div className="hero-copy"><div className="eyebrow"><span className="pulse" /> {t.hero.status}</div><h1>{t.hero.line1}<br /><em>{t.hero.accent}</em><br />{t.hero.line3}</h1><p className="hero-intro">{t.hero.intro}</p><div className="hero-actions"><a href="#proyectos" className="button button-primary">{t.hero.projects} <Arrow /></a><a href="#contacto" className="text-link">{t.hero.know} <Arrow /></a></div></div>
          <div className="hero-art" aria-label={t.hero.art}><div className="orbit orbit-one" /><div className="orbit orbit-two" /><div className="orbit orbit-three" /><div className="portrait-frame"><img src="/perfil-elihu.webp" alt={t.hero.photoAlt} width="900" height="900" decoding="async" fetchPriority="high" /><span className="portrait-shine" aria-hidden="true" /><span className="portrait-badge"><strong>ENAP</strong><small>IT ENGINEERING</small></span></div><span className="float-card card-cloud">CLOUD <strong>02</strong></span><span className="float-card card-code">&lt;/&gt;</span><span className="float-card card-network">◌ <strong>NETWORKS</strong></span></div><div className="scroll-cue"><span>{t.hero.scroll}</span><i /></div>
        </section>
